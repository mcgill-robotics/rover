...
class SerialInterface:
    ...
    def stop_link(self):
        ...
        """
        # End Communications
        ack = package_frame(self.SYS_ID, self.next_frame_id, '')
    ...
    def _receive_frame(self):
        ...
        frame_type = chr(int.from_bytes(self.mcu.read(1), "big"))
        frame = f'{frame}{frame_type}'
        if frame_type in ('A', 'R', 'S', 'Y', 'F', 'Q'):
            return frame

        # Get the payload and frame length
        payload_length = chr(int.from_bytes(self.mcu.read(1), "big"))
        frame_length = 3 + payload_length + 1

        # Get the supposed amount of bytes
        for byte in range(payload_length+1):
            read_byte = chr(int.from_bytes(self.mcu.read(1), "big"))
            frame = f'{frame}{read_byte}'

        if len(frame) != frame_length:
            raise ValueError  # Not the right number of bytes received

        # Check CRC8
        crc = 0
        for char in frame[3:-1]:
    ...
    def put_packet(self, frame_type, packet):
        ...
        """
        frame = package_frame(self.SYS_ID, frame_type, packet)
        self.next_frame_id = (self.next_frame_id + 1) % 32
...
def package_frame(sys_id, frame_type, payload=''):
    """Builds the frame following the protocol standard

    Parameters
    --------
    sys_id : str
        system id of the sender #TODO UNUSED BECAUSE ASSUMED IN PAYLOAD
    frame_type : str
    ...
    if frame_type in ('0', '1'):
        payload_length = 4 * length + 1  # Assumes system id is in the payload
    elif frame_type in '2':
        payload_length = 2 * length  # Pixels are two bytes
    else:
        payload_length = 0

    # Check if too big to send
    packet_size = 3 + payload_length + 1
    # if packet_size >= MAX_PACKET_SIZE:
    if packet_size >= 64:  # TODO define MAX_PACKET_SIZE
        return False
    ...
    if frame_type in ('0', '1'):  # Sending floats
        byte_array += payload[0].encode('ascii')  # Encode system ID (ASSUMING IT IS IN PAYLOAD)
        for float_value in payload[1:]:
            byte_array += float_to_bin(float_value)

    elif frame_type in '2':  # Sending pixels values
        pass  # TODO
...
def unpackage_frame(frame):
    ...
    if len(frame) > 2:
        ...
        payload = frame[3:-1]
        checksum = frame[-1]
    return frame_type, payload_length, payload, checksum
