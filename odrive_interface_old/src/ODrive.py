...
class Node_ODriveInterface():
    def __init__(self):
        ...
        self.active_errors = {"DRIVE_LB": 0,
                              "DRIVE_LF": 0, "DRIVE_RB": 0, "DRIVE_RF": 0}
        ...
        self.error_publisher = rospy.Publisher(
            "/odrive_error", MotorError, queue_size=1)
        self.state_publisher = rospy.Publisher(
            "/odrive_state", MotorState, queue_size=1)
        self.command_subscriber = rospy.Subscriber(
            "/wheel_velocity_cmd", WheelSpeed, self.handle_drive_command)
    ...
    def run(self):
        ...
        while not rospy.is_shutdown():
            ...
            for motor in drive_motors:
                state_fb = MotorState()
                state_fb.id = drive_ids[format(
                    motor.serial_number, "x").upper()]
                ...
                if motor.axis0.active_errors != 0:
                    ...
                    if motor.axis0.active_errors != 0:
                        error_fb = MotorError()
                        error_fb.id = drive_ids[format(
                            motor.serial_number, "x").upper()]
                        error_fb.error = decode_errors(
                            motor.axis0.active_errors)
