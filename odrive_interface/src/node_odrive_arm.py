...
class NodeODriveInterfaceArm:
            ...
            for joint_name, joint_obj in self.joint_dict.items():
                try:
                    if not joint_obj.odrv:
                        continue
                    temp = self.joint_pos_outshaft_dict[joint_name] * joint_obj.gear_ratiob / 360
                    joint_obj.odrv.axis0.set_abs_pos(temp)
                except:
                    print(f"""Cannot home joint: {joint_name} to position: {self.joint_pos_outshaft_dict[joint_name]}""")
    ...
    def run(self):
        ...
        for key, value in self.joint_serial_numbers.items():
            ...
            if value == 0:
                print(f"""Skipping connection for joint: {key} due to serial_number being 0""")
                continue
        ...
        while not rospy.is_shutdown():
            ...
            for joint_name, joint_obj in self.joint_dict.items():
                ...
                try:
                    ...
                    feedback.data.append(360 * (joint_obj.odrv.axis0.pos_vel_mapper.pos_abs / joint_obj.gear_ratio))
            ...
            for joint_name, joint_obj in self.joint_dict.items():
                ...
                try:
                    ...
                    print(f"""Cannot apply setpoint {setpoint} to joint: {joint_name}""")
                ...
                with self.locks[joint_name]:  # Use a lock specific to each joint
                    if joint_obj.odrv is not None:
                        ...
                        if hasattr(joint_obj.odrv, 'axis0') and joint_obj.odrv.axis0.current_state != AxisState.CLOSED_LOOP_CONTROL:
                            ...
                            print(f"Cannot check current state for {joint_name}, 'axis0' attribute missing.")
                    else:
