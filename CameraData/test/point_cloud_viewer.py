...
while True:
    ...
    rcd.grid(out, (0, 0.5, 1), size=1, n=10)
    rcd.frustum(out, depth_intrinsics)
    rcd.axes(out, rcd.view([0, 0, 0]), state.rotation, size=0.1, thickness=1)

    if not state.scale or out.shape[:2] == (h, w):
        rcd.pointcloud(out, verts, texcoords, color_source)
    else:
        tmp = np.zeros((h, w, 3), dtype=np.uint8)
        rcd.pointcloud(tmp, verts, texcoords, color_source)
        tmp = cv2.resize(
            tmp, out.shape[:2][::-1], interpolation=cv2.INTER_NEAREST)
        np.putmask(out, tmp > 0, tmp)

    if any(state.mouse_btns):
        rcd.axes(out, rcd.view(state.pivot), state.rotation, thickness=4)
