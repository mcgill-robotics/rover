...
class Node_GamepadProcessing:
    def __init__(self, v_max, w_max):
        ...
        self.camera_publisher = rospy.Publisher(
            "panTiltAngles", Float32MultiArray, queue_size=1
...
def keyboardProcessCall(self, key):
    ...
    self.roverLinearVelocity = max(
        -self.maxLinearVelocity, min(self.roverLinearVelocity, self.maxLinearVelocity)
    )
    ...
    def cameraProcessCall(self, msg):
        ...
        if (
            (msg.B3 == 1 or msg.B1 == 1)
            and (self.cam_ctrl.data[0] + msg.B3 <= 180)
            and (self.cam_ctrl.data[0] - msg.B1 >= 0)
        ):
            self.cam_ctrl.data[0] = self.cam_ctrl.data[0] + msg.B3 - msg.B1
            print(f"vertical: {self.cam_ctrl.data[0]}")

        if (
            (msg.B4 == 1 or msg.B2 == 1)
            and (self.cam_ctrl.data[1] + msg.B2 <= 180)
            and (self.cam_ctrl.data[1] - msg.B4 >= 0)
        ):
